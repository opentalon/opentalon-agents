...
func actions() []pkg.ActionMsg {
	...
	notify := pkg.ParameterMsg{
		Name:        "notify",
		Description: `Optional. Opt this agent into NOTIFYING when it fires (or, for a schedule agent, after each run) by PUSHING a message. Use this for "tell me / let me know / ping X when Y" — do NOT hardcode a chat id or author an mcp send step for it; every destination is resolved by the host and the LLM never sees an address. Costs no tokens and starts no assistant turn (that is ` + "`escalate`" + `). JSON object: {"enabled":true,"template":"...","recipients":[...],"channels":[...]} (or the shorthand "true"). template optionally overrides the message text (placeholders: {{agent_name}} {{description}} {{firings}} {{facts}} {{result}} {{error}} {{trigger}}). recipients selects WHO (default: the creator) — an array of {"kind":"creator"} (or "me"), {"kind":"responsible"} (the person responsible for each fired item — needs an id_field on the trigger), or {"kind":"role","role":"procurement"} (a named role/team). channels selects HOW — any of "in_app","email" (default: in_app). Targeting only responsible/role needs no creator conversation, so it works from a non-interactive context.`,
		Type:        "string",
		Required:    false,
	}
