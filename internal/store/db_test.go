...
func TestOpen_MigratesAndIsIdempotent(t *testing.T) {
	...
	for _, table := range []string{"agents", "runs", "agent_state", "pending_events", "agent_escalations", "agent_notifications"} {
	...
	if v, _ := db2.currentVersion(); v != 3 {
		t.Errorf("version should stay 3 on reopen, got %d", v)
	}
}
